...
def parse_monkey(monkey):
    ...
    for attr in monkey.split('\n'):
        ...
        # Get starting items
        elif attr[: 14] == 'Starting items':
            starting_items = attr[16:].split(', ')

        elif attr[:4] == 'Test':
            # Get the multiple of all the test mods
            global mod
            mod *= int(attr.split()[-1])
...
def monkey_turn(monkey):
    ...
    while items:
        ...
        item %= mod    # Remove multiples that are divisble by all the test cases to decrease the size of item
        if item % test == 0:
